  ...
  useEffect(() => {
    const fetchPlans = async () => {
      try {
        ...
        const response = await fetch(`${API_ENDPOINTS.PLANS}?display_on=house-plans`);
        ...
        if (response.ok) {
          ...
          const transformedPlans = plansList.map((plan: any) => {
            // Get all images: first the plan_images array, then the primary image
            const allImages = [
              ...(plan.plan_images?.map((img: any) => img.image_url) || []),
              ...(plan.image_url ? [plan.image_url] : [])
            ];
            const images = allImages.length > 0 ? allImages : [housePlans[0]?.images?.[0]];
            
            console.log(`Plan: ${plan.name}, Total Images: ${images.length}`, {
              plan_images_count: plan.plan_images?.length || 0,
              primary_image: plan.image_url ? 'yes' : 'no',
              all_images: images
            });
            ...
            return {
              ...
              features: plan.features?.map((f: any) => f.name) || ['Quality Build'],
              videoUrl: plan.video_url || 'https://www.youtube.com/embed/ciXvD_-rtts',
              enSuite: 1,
              ...
              coveredParking: 2,
              petFriendly: plan.pet_friendly || false,
              amenities: plan.amenities?.map((a: any) => a.name) || ['Built in cupboards'],
              floors: plan.floors || [],
            };
          });
