...
export const BuiltHomes = () => {
  ...
  useEffect(() => {
    const fetchBuiltHomes = async () => {
      try {
        setLoading(true);
        const response = await fetch(`${API_ENDPOINTS.PLANS}?display_on=built-homes`);
        const data = await response.json();
        
        // Handle both paginated and non-paginated responses
        const plansList = data.results || data;
        
        // Transform API response to match HousePlan type
        const transformedPlans = (Array.isArray(plansList) ? plansList : []).map((plan: any) => ({
          id: plan.id,
          title: plan.name,
          ...
          images: plan.plan_images?.map((img: any) => img.image) || [plan.image],
          ...
          coveredParking: 0,
          petFriendly: plan.pet_friendly,
          amenities: plan.amenities?.map((a: any) => a.name) || [],
        }));
        
        console.log('Fetched built homes:', transformedPlans);
